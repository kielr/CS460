...
_int80h:
    push bp ! save all Umode registers into ustack
    push si
    push di
    push es
    push ds
            ! Now the ustack contains |uFlag, uCS, uPC|bp,si,di,ues,uds|
            !                                                      ^usp
            ! Now we must change DS to KDS in order to access data in Kernal space
