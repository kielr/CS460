...
int fork()
{
  ...
  printf("pid=%d proc.pid=%d segment=%x active=%d\n", 
	 pid, p->pid, segment, p->active);
  ...
  *********************************************************/
   ...
   printList("readyQueue ", readyQueue);
   printf("Proc %d forked a child %d at segment=%x active=%d\n",
	  running->pid, p->pid, segment, p->active);
   return p->pid;
}
